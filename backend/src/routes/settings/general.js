...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (logo?.[0]) {
        settings.logo = `${process.env.SERVER_URL}:${process.env.NODE_ENV === 'production'?'':process.env.PORT}/uploads/${logo[0].filename}`;       
      }

      if (backgroundImage?.[0]) {
        settings.backgroundImage =  `${process.env.SERVER_URL}:${process.env.NODE_ENV === 'production'?'':process.env.PORT}/uploads/${backgroundImage[0].filename}`      
      }
